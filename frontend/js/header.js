...
function active(h){const p=(location.pathname.split('/').pop()||'index.html');return p===h}
export function mountHeader(){
  ...
  el.innerHTML=`<header class="site"><div class="header-inner"><a class="brand" href="index.html">Blin<span>kGames</span></a><nav class="nav">${LINKS.map(l=>`<a class="${active(l.href)?'active':''}" href="${l.href}">${l.label}</a>`).join('')}${right}</nav></div></header>`;
