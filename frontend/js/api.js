...
async function request(path, method='GET', data=null, token=null){
  ...
  if(data) opt.body=JSON.stringify(data); if(token) opt.headers.Authorization=`Bearer ${token}`;
  const r=await fetch(url,opt); let j=null; try{j=await r.json()}catch{j={}}
  if(!r.ok) throw new Error(j?.message||r.statusText); return j;
}
